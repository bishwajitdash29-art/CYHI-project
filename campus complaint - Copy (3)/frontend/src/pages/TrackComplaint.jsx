    ...
    <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-8 border dark:border-gray-700 rounded-lg shadow-sm mt-10">
      ...
      {complaint && (
        <div className="space-y-6">
          <div className="border dark:border-gray-700 rounded-lg p-6 bg-gray-50 dark:bg-gray-900">
          ...
          <div className="mt-8">
            ...
            <div className="space-y-4">
              {complaint.replies.map(reply => (
                <div key={reply.id} className={`p-4 rounded-lg border dark:border-gray-700 ${reply.user?.role === 'STUDENT' ? 'bg-blue-50 dark:bg-blue-900/20 ml-8' : 'bg-gray-50 dark:bg-gray-800 mr-8'}`}>
          ...
          {complaint.status === 'AWAITING_REVIEW' && (
            <div className="mt-8 p-6 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg text-center">
              <h4 className="text-xl font-bold text-purple-900 dark:text-purple-300 mb-2">Review Required</h4>
              <p className="text-purple-800 dark:text-purple-400 mb-6">The assigned authority has responded to your complaint. Is your issue resolved?</p>
              <div className="flex gap-4 justify-center">
                <button 
                  onClick={async () => {
                    const res = await axios.post(`http://localhost:5000/api/complaints/track/${searchToken}/review`, { resolution: 'RESOLVED' });
                    setComplaint(res.data);
                  }}
                  className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded shadow-sm transition"
                >
                  Issue Resolved
                </button>
                <button 
                  onClick={async () => {
                    const res = await axios.post(`http://localhost:5000/api/complaints/track/${searchToken}/review`, { resolution: 'REOPENED' });
                    setComplaint(res.data);
                  }}
                  className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded shadow-sm transition"
                >
                  Not Resolved (Reopen)
                </button>
              </div>
