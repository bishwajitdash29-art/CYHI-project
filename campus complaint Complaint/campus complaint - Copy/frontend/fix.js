...
export default function Proposals() {
  ...
  const handleUpvote = async (id) => {
    ...
    try {
      await axios.post(\`http://localhost:5000/api/proposals/\${id}/upvote\`, {}, {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) return;
    try {
      await axios.post("http://localhost:5000/api/proposals", { title, description }, {
        headers: { Authorization: \`Bearer \${token}\` }
      });
      setTitle("");
      setDescription("");
  ...
  const handleStatus = async (id, status) => {
    try {
      await axios.put(\`http://localhost:5000/api/proposals/\${id}/status\`, { status }, {
        headers: { Authorization: \`Bearer \${token}\` }
      });
      fetchProposals();
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto mt-10">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2">Suggest new ideas and upvote the best ones!</p>
        </div>
        {isStudent && (
          <button 
      ...
      {showSubmit && (
        <div className="linear-card p-6 mb-8 border-indigo-500/30">
          <h3 className="text-xl font-bold dark:text-white mb-4">Submit an Idea</h3>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                      Suggested by <span className="font-medium">{p.author?.name}</span> • {new Date(p.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                  <div>
...
fs.writeFileSync("c:/Users/utkvi/Downloads/campus complaint/frontend/src/pages/Proposals.jsx", code);
