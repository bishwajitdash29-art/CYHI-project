...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    let myTokens = [];
    try {
      myTokens = JSON.parse(localStorage.getItem('my_complaints') || '[]');
    } catch (e) {
      myTokens = [];
    }
    const tokenQuery = myTokens.length > 0 ? `?tokens=${myTokens.join(',')}` : '';

    axios.get(`${API_URL}/api/complaints${tokenQuery}`, {
    ...
    axios.get(API_URL + '/api/complaints/escalated', {
      headers: { Authorization: `Bearer ${token}` }
    })
  ...
  const handleReply = async (complaintId) => {
    ...
    const token = localStorage.getItem('token') || sessionStorage.getItem('token');
    try {
      await axios.post(`${API_URL}/api/complaints/${complaintId}/reply`, 
        { message: replyText },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setReplyText('');
  ...
  const handleStatusChange = async (complaintId, status) => {
    const token = localStorage.getItem('token') || sessionStorage.getItem('token');
    try {
      await axios.put(`${API_URL}/api/complaints/${complaintId}/status`, 
        { status },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handlePing = async (complaintId) => {
    const token = localStorage.getItem('token') || sessionStorage.getItem('token');
    try {
      await axios.post(`${API_URL}/api/complaints/${complaintId}/ping`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="space-y-6 mt-10 max-w-5xl mx-auto">
      ...
      <div className="grid gap-4">
        {complaints.length === 0 ? (
          ...
          return (
          <div key={c.id} className={`p-6 md:p-8 transition-colors ${c.status === 'REOPENED' ? 'bg-orange-900/10 border-orange-500/30 linear-card' : isPingedByDirector ? 'bg-red-900/10 border-red-500/30 linear-card' : 'linear-card'}`}>
            <div className="flex justify-between items-start border-b border-gray-100 dark:border-white/5 pb-4 mb-4">
              <div>
                <h3 className="font-bold text-xl text-gray-900 dark:text-white flex items-center gap-3">
                  {c.title}
                  {c.status === 'REOPENED' && <span className="bg-orange-500/10 text-orange-500 border border-orange-500/20 font-bold tracking-widest uppercase text-[10px] px-3 py-1 rounded-full">NOT RESOLVED</span>}
                  {isPingedByDirector && <span className="bg-red-500/10 text-red-500 border border-red-500/20 font-bold tracking-widest uppercase text-[10px] px-3 py-1 rounded-full animate-pulse">ESCALATED: PINGED</span>}
                </h3>
                <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mt-2 flex flex-wrap items-center gap-3">
                  <span className="bg-gray-100 dark:bg-white/5 px-2 py-1 rounded">ID: <span className="font-mono">{c.trackingToken}</span></span>
                  <span>{new Date(c.createdAt).toLocaleDateString()}</span>
                  {(c.status === 'PENDING' || c.status === 'REOPENED') && (
                    <span className={`px-2 py-1 rounded ${isOverdue ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-green-500/10 text-green-400 border border-green-500/20'}`}>
                      {slaStatus}
                    </span>
                  )}
              ...
              {c.status === 'AWAITING_REVIEW' ? (
                <span className="bg-purple-500/10 text-purple-400 border border-purple-500/20 font-bold tracking-widest uppercase text-[10px] px-3 py-1 rounded-full">
                  Sent for review
                </span>
              ) : (
                <select 
                  ...
                  className="bg-transparent border border-white/10 hover:border-white/20 text-gray-300 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-widest outline-none transition-colors cursor-pointer appearance-none text-center"
                >
                  <option value="PENDING" className="bg-gray-900 text-white">Pending</option>
                  <option value="IN_PROGRESS" className="bg-gray-900 text-white">In Progress</option>
                  <option value="AWAITING_REVIEW" className="bg-gray-900 text-white">Awaiting Review</option>
                  <option value="REOPENED" className="bg-gray-900 text-white">Reopened</option>
