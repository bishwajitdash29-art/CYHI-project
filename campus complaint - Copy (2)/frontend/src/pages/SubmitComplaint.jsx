...
export default function SubmitComplaint() {
  ...
  return (
    <div className="max-w-lg mx-auto bg-white dark:bg-gray-800 p-8 border dark:border-gray-700 rounded-lg shadow-sm mt-10">
      <button onClick={() => setStep(1)} className="flex items-center text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 mb-6 transition">
        <ArrowLeft size={20} className="mr-2" /> Back to Categories
      </button>
      
      <h2 className="text-2xl font-bold mb-6 dark:text-white">Submit Complaint to {roleNames[targetRole]}</h2>
      {error && <div className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 p-3 rounded mb-4">{error}</div>}
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block font-medium mb-1 dark:text-gray-200">Select specific {roleNames[targetRole]}</label>
          <select 
          ...
          {targets.length === 0 && (
            <p className="text-sm text-yellow-600 dark:text-yellow-400 mt-1">No registered users found in this category.</p>
          )}
        </div>
        <div>
        ...
        <div>
          ...
          <textarea 
            className="w-full border dark:border-gray-600 bg-white dark:bg-gray-700 dark:text-white p-2 rounded focus:ring-blue-500 focus:border-blue-500 h-32" 
            value={description}
